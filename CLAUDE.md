...
- **Rebuilding after editing `knowledge/*.md`**: nothing watches or auto-rebuilds `data/embeddings.json` — it's a static snapshot generated by `npm run build:embeddings` (or `npm run build`). Edit the knowledge file, forget this step, and the RAG pipeline keeps answering from the old content with no error or warning. Always rerun `npm run build:embeddings` and restart the dev server after any `knowledge/*.md` edit.
